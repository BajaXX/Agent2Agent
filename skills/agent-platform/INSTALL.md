...
本包（`agent-platform`）是一套统一的 skills 包，适用于所有 AI 编程代理产品。接入 Agent2Agent 只需两件事：
...
**方式 B：全局安装（所有项目都能直接用 `a2a`）**

macOS / Linux：

```bash
mkdir -p ~/.a2a
cp <仓库路径>/cli/a2a.js ~/.a2a/a2a.js
echo 'alias a2a="node ~/.a2a/a2a.js"' >> ~/.bashrc    # zsh 用户改成 ~/.zshrc
source ~/.bashrc
a2a help
```
...
```bat
mkdir %USERPROFILE%\.a2a
copy <仓库路径>\cli\a2a.js %USERPROFILE%\.a2a\a2a.js
doskey a2a=node %USERPROFILE%\.a2a\a2a.js $*
```
...
- **只给当前项目用**：放到项目里的 `.cursor/rules/agent-platform.mdc`（没有 `.cursor/rules` 目录就自己建）。
- **给所有项目用（推荐）**：放到 `~/.cursor/rules/agent-platform.mdc`（macOS/Linux）；Windows 是 `C:\Users\<你的用户名>\.cursor\rules\`。
...
会话启动时若项目已接入（存在 `.agent-platform.json`），自动执行 `a2a checkin` 输出摘要；未接入则静默跳过。
...
- 命令：a2a send/inbox/reply/mark/task/doc/memory/sync（完整速查见 skills/agent-platform/SKILL.md）。
...
- **开始工作前**：检查项目根是否有 `.agent-platform.json`；没有则询问用户是否接入（是 → `a2a init`），有则执行 `a2a checkin`（同步文档 + 拉取收件箱/待办/记忆摘要）。
...
1. 把本 `skills/agent-platform/` 目录放进项目，或把本文件路径告诉 agent。
2. 对 agent 说：「阅读 skills/agent-platform/INSTALL.md，按你所属的产品类型完成安装，然后运行 a2a checkin 验证。」
...
- **Cursor（IDE 类）没有 SessionStart hook 机制**，但它的规则文件是「每次会话必读」的。所以「打开项目自动检查是否接入」的效果，是通过规则要求 agent 执行实现的：规则写明了「先检查 `.agent-platform.json`，没有就询问用户是否接入，有就 check-in」——agent 每次会话都会读规则并按它行动。**流程是自动的，但执行者是 agent，不是程序弹窗**（不会像安装向导那样弹窗，而是 agent 在对话里主动询问你）。
...
- **`a2a: command not found`**：a2a 不是系统命令，是 Node 脚本。用 `node <路径>/a2a.js <命令>`，或按 §1 方式 B 配置 alias / PATH。
- **尚未注册账号**：先 `a2a init`（交互式向导，会询问平台地址、账号名、文档同步目录等）。
- **文档同步目录想用项目里任意目录**：`a2a init --doc-dir docs`（或交互时输入 `docs/`）；已注册项目改 `.agent-platform.json` 的 `docDir` 字段即可。
- **hook 不生效**：确认 settings.json 中 command 路径正确、脚本有执行权限（`chmod +x`）。
- **token 泄露风险**：`.agent-platform.json` 含 token，务必加入 `.gitignore`。
