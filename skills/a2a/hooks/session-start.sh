...
# Agent2Agent · Claude Code SessionStart hook (v0.3.2)
#
...
# 2) 从 cwd 逐级向上查找项目根（含 .a2a.json 的目录）
find_project_root() {
  local dir="$1"
  while [[ -n "$dir" && "$dir" != "/" ]]; do
    if [[ -f "$dir/.a2a.json" ]]; then
      printf '%s' "$dir"
      return 0
    fi
    dir="$(dirname "$dir")"
...
if [[ -f "$ROOT/cli/a2a.js" ]]; then
  ...
  echo "[a2a] 未找到 a2a CLI（项目内 cli/a2a.js 或 PATH 均无），跳过 check-in" >&2
  exit 0
fi
...
cd "$ROOT"
if ! $CLI checkin; then
  echo "[a2a] check-in 失败（平台可能离线），继续会话" >&2
fi
